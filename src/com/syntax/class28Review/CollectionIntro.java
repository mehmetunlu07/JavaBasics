package com.syntax.class28Review;
...
//		it.remove(); //will remove the element while iterating
		...
		asia.add("Tajikistan");
		asia.add("Japan");
		asia.add("China");		//duplicate
		asia.add("Turkey");
		asia.add("Nepal");
		
		System.out.println(asia);   
		
		Set<String> asia1=new LinkedHashSet<>(asia);    // 1.yontem duplicate leri cikarmak
		System.out.println(asia1);
		
		System.out.println("------------------------------");
		
		List<String>asiaNew=new ArrayList<>();
		
		Iterator<String>itr=asia.iterator();           // 2.yontem duplicate leri cikarmak
		while(itr.hasNext()){
			String country=itr.next();
			if(!asiaNew.contains(country)) {
				asiaNew.add(country);
			}
		}
		System.out.println(asiaNew);
	}
